...
run_desc += "-ixi"
train.update(
    dataset_train=dict(fn="ixi_train.pkl"), augment_params=dict(translate=64)
)  # 4936 images, lots of augmentation.
...
if train.get("num_epochs"):
    run_desc += "-ep%d" % train["num_epochs"]
if train.get("learning_rate_max"):
    run_desc += "-lr%f" % train["learning_rate_max"]
if not train.get("corrupt_targets", True):
    run_desc += "-clean"
if train.get("minibatch_size"):
    run_desc += "-mb%d" % train["minibatch_size"]
if train["corrupt_params"].get("type") == "gaussian":
    run_desc += "-Cg%f" % train["corrupt_params"]["scale"]
if train["corrupt_params"].get("type") == "bspec":
    run_desc += "-Cbs%f" % train["corrupt_params"]["p_at_edge"]
if train["corrupt_params"].get("type") == "bspeclin":
    run_desc += "-Cbslin%f" % train["corrupt_params"]["p_at_edge"]
if train["augment_params"].get("translate", 0) > 0:
    run_desc += "-At%d" % train["augment_params"]["translate"]
if train.get("post_op"):
    run_desc += "-P%s" % train["post_op"]
if random_seed != 1000:
    run_desc += "-%d" % random_seed
if train.get("load_network"):
    run_desc += "-LOAD%s" % train["load_network"][:3]
if train.get("start_epoch"):
    run_desc += "-start%s" % train["start_epoch"]
