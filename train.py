...
class AugmentGaussian:
    ...
    def add_train_noise_tf(self, x):
        ...
        shape = tf.shape(x)
        rng_stddev = tf.random_uniform(
            shape=[1, 1, 1], minval=minval / 255.0, maxval=maxval / 255.0
        )
...
class AugmentImpulse:
    ...
    def add_train_noise_tf(self, x):
        ...
        alpha = tf.random_uniform(
            ...
            minval=self.alpha_min,
            maxval=self.alpha_max,
        )
        keep_mask = tf.where(
            tf.random_uniform(shape=msh) >= tf.ones(shape=msh) * alpha,
            tf.ones(shape=msh),
            tf.zeros(shape=msh),
        )
    ...
    def add_validation_noise_np(self, x):
        ...
        alpha = 0.5
        keep_mask = np.where(
            np.random.uniform(size=msh) >= np.ones(shape=msh) * alpha,
            np.ones(shape=msh),
            np.zeros(shape=msh),
        )
...
def train(
    ...
    ctx.update(
        loss="run %d" % submit_config.run_id, cur_epoch=0, max_epoch=iteration_count
    )
    ...
    for i in range(iteration_count):
        ...
        if i % eval_interval == 0:
            ...
            ctx.update(
                loss="run %d" % submit_config.run_id,
                cur_epoch=i,
                max_epoch=iteration_count,
            )
            time_maintenance = ctx.get_last_update_interval() - time_train

        lrate = compute_ramped_down_lrate(
            i, iteration_count, ramp_down_perc, learning_rate
        )
