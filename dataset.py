...
def resize_small_image(x):
    shape = tf.shape(x)
    return tf.cond(
        tf.logical_or(tf.less(shape[2], 256), tf.less(shape[1], 256)),
        true_fn=lambda: hwc_to_chw(
            tf.image.resize_images(
                chw_to_hwc(x), size=[256, 256], method=tf.image.ResizeMethod.BICUBIC
            )
        ),
...
def create_dataset(train_tfrecords, minibatch_size, add_noise):
    ...
    dset = tf.data.TFRecordDataset(
        train_tfrecords, compression_type="", buffer_size=buffer_mb << 20
    )
    dset = dset.repeat()
    buf_size = 1000
