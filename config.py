...
poisson_noise_config = dnnlib.EasyDict(func_name="train.AugmentPoisson", lam_max=50.0)

impulse_noise_config = dnnlib.EasyDict(
    func_name="train.AugmentImpulse", alpha_min=0, alpha_max=1
)
# ------------------------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    def validate(args):
        ...
        if args.dataset_dir is None:
            ...
            validate_config.dataset = {"dataset_dir": args.dataset_dir}
        if args.noise not in corruption_types:
            error("Unknown noise type", args.noise)
    ...
    parser.add_argument(
        "--desc", default="", help="Append desc to the run descriptor string"
    )
    parser.add_argument(
        "--run-dir-root",
    ...
    parser_train.add_argument(
        "--noise",
        default="gaussian",
        help="Type of noise corruption (one of: gaussian, poisson, impulse)",
    )
    parser_train.add_argument(
        "--long-train",
        default=False,
    ...
    parser_train.add_argument(
        "--train-tfrecords", help="Filename of the training set tfrecords file"
    )
    parser_train.set_defaults(func=train)

    parser_validate = subparsers.add_parser(
        "validate", help="Run a set of images through the network"
    )
    ...
    parser_infer_image = subparsers.add_parser(
        "infer-image", help="Run one image through the network without adding any noise"
    )
