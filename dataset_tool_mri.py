...
def genpng(args):
    ...
    for nii_file in nii_files:
        ...
        print("name", name)
        hborder = (
            np.asarray([OUT_RESOLUTION, OUT_RESOLUTION]) - nii_img.shape[0:2]
        ) // 2
        ...
        for s in range(slice_min, slice_max):
            ...
            output[
                hborder[0] : hborder[0] + nii_img.shape[0],
                hborder[1] : hborder[1] + nii_img.shape[1],
            ] = slice
...
def genpkl(args):
    ...
    for base in train_basenames:
        for sidx in range(slice_min, slice_max):
            input_train_files.append(
                os.path.join(args.png_dir, make_slice_name(base, sidx))
            )
    for base in valid_basenames:
        for sidx in range(slice_min, slice_max):
            input_valid_files.append(
                os.path.join(args.png_dir, make_slice_name(base, sidx))
            )
    ...
    preprocess_mri(
        ...
        output_file=os.path.join(args.pkl_dir, "ixi_train.pkl"),
    )
    preprocess_mri(
        input_files=input_valid_files,
        output_file=os.path.join(args.pkl_dir, "ixi_valid.pkl"),
    )
...
def main():
    ...
    parser_genpng = subparsers.add_parser(
        "genpng", help="IXI nifti to PNG converter (intermediate step)"
    )
    parser_genpng.add_argument(
        "--ixi-dir", help="Directory pointing to unpacked IXI-T1.tar"
    )
