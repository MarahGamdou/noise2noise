...
def get_weight(shape, gain=np.sqrt(2)):
    ...
    w = tf.get_variable(
        "weight", shape=shape, initializer=tf.initializers.random_normal(0, std)
    )
    return w
...
def conv2d_bias(x, fmaps, kernel, gain=np.sqrt(2)):
    ...
    w = tf.cast(w, x.dtype)
    return apply_bias(
        tf.nn.conv2d(x, w, strides=[1, 1, 1, 1], padding="SAME", data_format="NCHW")
    )
...
def maxpool2d(x, k=2):
    ...
    return tf.nn.max_pool(
        x, ksize=ksize, strides=ksize, padding="SAME", data_format="NCHW"
    )
